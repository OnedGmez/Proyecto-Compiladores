/*
	...
	# Tabla de símbolos: Inicialmente contendrá cargadas las palabras resevardas y adicionalmente la utilizaremos para almacenar las variables definidas por el usuario
		- Estructura para palabras reservadas: Nombre, Tipo, NE, NE
			* NE: NO ESPECIFICADO
		- Estructura para variables de usuario: Nombre, Tipo, Linea(declarada), Lineas(Uso)
*/
...
func (h *hashtable) insertInHashTable(key [1][4]string) {
	index := hash(key[0][0])
	h.array[index].insertBucketNode(key)
}

func (b *bucket) insertBucketNode(k [1][4]string) {
...
func main() {
	...
	var datos = [1][4]string{{"&(", "INT", "4", "12"}}
	tablaSimbolos.insertInHashTable(datos)
	datos = [1][4]string{{"''", "INT", "4", "12"}}
	tablaSimbolos.insertInHashTable(datos)
